...
const torch::Tensor toMashSamplePoints(
    ...
    const bool &use_inv, const bool &return_inner,
    const bool &return_boundary) {
  torch::Tensor inner_points;
  torch::Tensor boundary_points;
  ...
  if (return_inner) {
    ...
    const torch::Tensor in_max_mask_base_values = sample_base_values.index(
        {slice_all, in_max_mask_sample_polar_data_idxs});
    ...
    const torch::Tensor in_mask_sample_theta_weights =
        toInMaskSampleThetaWeights(in_max_mask_sample_thetas,
                                   in_max_mask_thetas,
    ...
    const torch::Tensor in_mask_sh_points =
        toSHPoints(sh_params, rotate_vectors, positions, sample_sh_directions,
                   ...
                   in_mask_sample_polar_data_idxs, use_inv);
  ...
  if (return_boundary) {
    ...
    const torch::Tensor mask_boundary_sh_points =
        toSHPoints(sh_params, rotate_vectors, positions, sample_sh_directions,
                   ...
                   mask_boundary_phi_data_idxs, use_inv);
  }

  const torch::Tensor sample_points =
      torch::vstack({inner_points, boundary_points});
