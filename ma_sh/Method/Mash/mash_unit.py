...
def toPreLoadMaskBoundaryPhiIdxs(
    anchor_num: int, mask_boundary_sample_num: int, device: str
):
    mask_boundary_phi_counts = (
        torch.ones(anchor_num).type(torch.int64).to(device) * mask_boundary_sample_num
    )
    mask_boundary_phi_idxs = mash_cpp.toIdxs(mask_boundary_phi_counts)

    assert checkFormat(
        mask_boundary_phi_counts, torch.int64, device, [anchor_num], False
    )
    assert checkFormat(
