...
def getMetrics(
    ...
    query_pts = (
        torch.from_numpy(np.asarray(query_pts)).unsqueeze(0).type(dtype).to(device)
    )
    gt_normals = (
        torch.from_numpy(np.asarray(gt_normals)).unsqueeze(0).type(dtype).to(device)
    )
    query_normals = (
        torch.from_numpy(np.asarray(query_normals)).unsqueeze(0).type(dtype).to(device)
    )

    dists2_1, dists2_2, idxs_1 = ChamferDistances.namedAlgo("cuda")(query_pts, gt_pts)[
        :3
    ]

    pair_idxs = torch.stack(
        [
            torch.arange(idxs_1.shape[1], dtype=torch.int64, device=device).expand(
                idxs_1.shape[0], idxs_1.shape[1]
            ),
            idxs_1.type(torch.int64),
        ],
        dim=-1,
    )
    ...
    for chamfer_order in chamfer_order_list:
        chamfer_dict[str(chamfer_order)] = toChamfer(dists2_1, dists2_2, chamfer_order)[
            0
        ].item()
    fscore_dict = {}
    for fscore_thresh in fscore_thresh_list:
        fscore_dict[str(fscore_thresh)] = toFScore(dists2_1, dists2_2, fscore_thresh)[
            0
        ].item()
...
def recordMetrics(
    ...
    for root, _, files in os.walk(query_data_folder_path):
        for file in files:
            ...
            rel_file_basepath = (
                os.path.relpath(root, query_data_folder_path) + "/" + file[:-4]
            )
            ...
            if query_mode == "mesh":
                ...
                query_pcd = Mash.fromParamsFile(
                    query_file_path, device="cuda"
                ).toSamplePcdWithWNNCNormals()
                query_pts = np.asarray(query_pcd.points)
                query_normals = np.asarray(query_pcd.normals)
            elif query_mode == "mashmesh":
                query_mesh = Mash.fromParamsFile(
                    query_file_path, device="cuda"
                ).toWNNCMesh()
            ...
            for key, item in metric_dict[rel_file_basepath].items():
                print(key, ":", item, "\t", end="")
            print()
            tmp_save_metric_file_path = save_metric_file_path.replace(
                ".npy", "_tmp.npy"
            )
