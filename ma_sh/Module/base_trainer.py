...
from ma_sh.Method.sample import downSample
...
class BaseTrainer(ABC):
    ...
    def loadParams(
        ...
        self.mash.loadParams(mask_params, sh_params, ortho_poses, positions, use_inv)
        return True
    ...
    def trainStep(
        ...
        start = time()
        boundary_pts, inner_pts = self.mash.toSamplePointsTuple()[:2]
    ...
    def toSamplePointsWithTransform(self) -> torch.Tensor:
        ...
        mask_boundary_sample_points, in_mask_sample_points, _ = (
            sample_mash.toSamplePointsTuple()
        )
