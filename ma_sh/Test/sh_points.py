...
def toParams(anchor_num: int, mask_degree_max: int, sh_degree_max: int, dtype, device: str):
    mask_params = (
...
def toPreLoadMaskBoundary(anchor_num: int, mask_boundary_sample_num: int, idx_dtype, dtype, device: str):
...
def toPreLoadBaseValues(mask_degree_max: int, sample_phis: torch.Tensor, mask_boundary_phis: torch.Tensor):
    ...
    if DEBUG:
        ...
        assert checkFormat(
            ...
            dtype,
            device,
            [mask_degree_max * 2 + 1, mask_boundary_phis.shape[0]], False
        )
        assert checkFormat(
            sample_base_values,
            dtype,
            device,
            [mask_degree_max * 2 + 1, sample_phis.shape[0]], False
        )
...
def toPreLoadRotateMatrixs(rotate_vectors: torch.Tensor):
    ...
    if DEBUG:
        ...
        assert checkFormat(
            ...
            dtype,
            device,
            [rotate_vectors.shape[0], 3, 3], True
        )
...
def toPreLoadSHDirections(sample_phis: torch.Tensor, sample_thetas: torch.Tensor):
    ...
    if DEBUG:
        ...
        assert checkFormat(
            ...
            dtype,
            device,
            [sample_phis.shape[0], 3], False
        )

    return sample_sh_directions

def toMaskBoundaryThetas(mask_params: torch.Tensor, mask_boundary_base_values: torch.Tensor, mask_boundary_phi_idxs: torch.Tensor):
    with torch.no_grad():
    ...
    if DEBUG:
        ...
        assert checkFormat(
            mask_boundary_thetas, dtype, device, [mask_boundary_base_values.shape[1]], False
        )

    return mask_boundary_thetas

def toInMaxMaskSamplePolars(sample_phis, sample_thetas, mask_boundary_thetas, mask_boundary_phi_idxs):
    ...
    if DEBUG:
        ...
        assert checkFormat(in_max_mask_sample_polar_counts, in_max_mask_sample_polar_idxs_list[0].dtype, device, None, False)
        assert checkFormat(
    ...
    return in_max_mask_sample_polar_idxs, in_max_mask_sample_polar_data_idxs, in_max_mask_sample_phis, in_max_mask_sample_thetas

def toInMaskSamplePolarWeights(mask_params, sample_base_values, in_max_mask_sample_phis, in_max_mask_sample_thetas, in_max_mask_sample_polar_idxs, in_max_mask_sample_polar_data_idxs):
    ...
    if DEBUG:
        ...
        assert checkFormat(
            ...
            dtype,
            device,
            [sample_base_values.shape[0], in_max_mask_sample_phis.shape[0]], False
        )
        assert checkFormat(
    ...
    return in_mask_sample_phis, in_mask_sample_polar_idxs, in_mask_sample_polar_data_idxs, in_mask_base_values, in_mask_sample_theta_weights

def toSamplePolars(mask_params, in_mask_base_values, in_mask_sample_polar_idxs, in_mask_sample_theta_weights):
...
def toSHValues(sh_degree_max, sh_params, in_mask_sample_phis, detect_thetas, in_mask_sample_polar_idxs):
...
def toSHPoints(rotate_matrixs, positions, sample_sh_directions, sh_values, in_mask_sample_polar_idxs, in_mask_sample_polar_data_idxs):
...
def test():
    ...
    mask_params, sh_params, rotate_vectors, positions = toParams(anchor_num, mask_degree_max, sh_degree_max, dtype, device)
    sample_phis, sample_thetas = toPreLoadUniformSamplePolars(sample_polar_num, dtype, device)
    mask_boundary_phi_idxs, mask_boundary_phis = toPreLoadMaskBoundary(anchor_num, mask_boundary_sample_num, idx_dtype, dtype, device)
    mask_boundary_base_values, sample_base_values = toPreLoadBaseValues(mask_degree_max, sample_phis, mask_boundary_phis)
    rotate_matrixs = toPreLoadRotateMatrixs(rotate_vectors)
    sample_sh_directions = toPreLoadSHDirections(sample_phis, sample_thetas)
    mask_boundary_thetas = toMaskBoundaryThetas(mask_params, mask_boundary_base_values, mask_boundary_phi_idxs)
    in_max_mask_sample_polar_idxs, in_max_mask_sample_polar_data_idxs, in_max_mask_sample_phis, in_max_mask_sample_thetas = toInMaxMaskSamplePolars(sample_phis, sample_thetas, mask_boundary_thetas, mask_boundary_phi_idxs)
    in_mask_sample_phis, in_mask_sample_polar_idxs, in_mask_sample_polar_data_idxs, in_mask_base_values, in_mask_sample_theta_weights = toInMaskSamplePolarWeights(mask_params, sample_base_values, in_max_mask_sample_phis, in_max_mask_sample_thetas, in_max_mask_sample_polar_idxs, in_max_mask_sample_polar_data_idxs)
    detect_thetas = toSamplePolars(mask_params, in_mask_base_values, in_mask_sample_polar_idxs, in_mask_sample_theta_weights)
    sh_values = toSHValues(sh_degree_max, sh_params, in_mask_sample_phis, detect_thetas, in_mask_sample_polar_idxs)
    sh_points = toSHPoints(rotate_matrixs, positions, sample_sh_directions, sh_values, in_mask_sample_polar_idxs, in_mask_sample_polar_data_idxs)
